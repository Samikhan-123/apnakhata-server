...
// This endpoint is hit by Vercel Cron (GET) OR manual Admin trigger (POST).
// Optional auth allows session detection for admins without blocking crons.
router.all(
  "/maintenance",
  authenticateOptional,
  maintenanceController.runMaintenance,
);
...
router.post(
  "/impersonate/stop",
  adminOnlyGuard,
  adminController.stopImpersonate,
);
router.post(
...
router.patch(
  ...
  adminOnlyGuard,
  adminController.scheduleDeletion,
);
router.patch(
  "/users/:id/cancel-deletion",
  adminOnlyGuard,
  adminController.cancelDeletion,
);
